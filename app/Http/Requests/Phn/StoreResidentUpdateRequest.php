...
class StoreResidentUpdateRequest extends FormRequest
{
    ...
    public function withValidator($validator): void
    {
        $validator->after(function ($validator): void {
            ...
            if (! $resident) {
                $validator->errors()->add('subject_id', 'Select a verified resident from the municipal registry.');
            }

            if (! $household) {
                $validator->errors()->add('household_id', 'Select a verified household from the municipal registry.');
            }
            ...
            $residentBarangayId = (int) $resident->household?->purok?->barangay_id;
            $householdBarangayId = (int) $household->purok?->barangay_id;

            if ($residentBarangayId !== $householdBarangayId) {
                $validator->errors()->add('household_id', 'For phase 1, PHN correction requests must stay inside the resident\'s current barangay so they route to the correct Secretary queue.');
            }
