...
@section('content')
    ...
    <div class="grid gap-6 xl:grid-cols-2">
        <section class="rounded-[24px] border border-slate-200 bg-white shadow-sm">
            ...
            <div class="p-6">
                <form method="POST" action="{{ route('bns.team.password.reset', $bhw) }}" class="space-y-4">
                    @csrf
                    @method('PUT')
        ...
        <section class="rounded-[24px] border border-slate-200 bg-white shadow-sm">
            ...
            <div class="p-6">
                <p class="text-sm leading-7 text-slate-600">
                    Generate a random temporary password for this BHW. The current password will be replaced immediately.
                </p>

                <form method="POST" action="{{ route('bns.team.password.generate', $bhw) }}" class="mt-5">
                    @csrf
                ...
                <div class="mt-5 rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900">
                    Make sure the temporary password reaches the BHW through a secure channel before they attempt to log in again.
                </div>
            </div>
        </section>
    </div>
